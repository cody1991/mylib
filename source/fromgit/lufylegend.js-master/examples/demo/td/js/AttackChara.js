...
AttackChara.prototype.onframe = function (){
	...
	//事件开始
	self.frameRun = true;
	if(self.attackTarget == null || self.attackTarget == "undefined"){
		self.findEnemy();
	}
	//事件结束
	self.frameRun = false;
...
AttackChara.prototype.hertCheck = function (anime){
	...
	if(self.attackTarget == null)return;
	//判断敌军是否已经移除攻击范围
	if(self.attackTarget.isDie || self.attackTarget.x < self.x - STEP || self.attackTarget.x > self.x + STEP || self.attackTarget.y < self.y  - STEP || self.attackTarget.y > self.y + STEP){
		self.attackTarget = null;
		self.anime.setAction(self.direction);
		self.anime.removeEventListener(LEvent.COMPLETE,self.hertCheck);
	}else{ 
		//重设攻击方向
		if((self.direction == LEFT || self.direction == RIGHT) && self.attackTarget.x >= self.x && self.attackTarget.x < self.x + STEP){
			if(self.attackTarget.y < self.y){
				self.direction = UP;
				self.anime.setAction(self.direction + 4);
			}else{
				self.direction = DOWN;
		...
		}else if((self.direction == UP || self.direction == DOWN) && self.attackTarget.y >= self.y && self.attackTarget.y < self.y + STEP){
			if(self.attackTarget.x < self.x){
				self.direction = LEFT;
				self.anime.setAction(self.direction + 4);
			}else{
				self.direction = RIGHT;
